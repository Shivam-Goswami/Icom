...
const clearCompleteTasksBtn = document.querySelector("button#clear-complete-tasks")
    ...
    const element = String.raw`
				<li class="ui segment grid equal width">
					<div class="ui checkbox column">
						<input id=${task.id} type="checkbox" ${task.completed ? "checked" : ""} onclick="completeTask(this.id)">
...
function clearCompleteTasks() {
  ...
  if (list.length > 0) {
    if (confirm("Are you sure?")) {
      ...
      // const filteredTasks= list.map((item,index)=>{
      //   if(item.)
      // })
